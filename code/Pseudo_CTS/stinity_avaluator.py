...
sitiny_repo = os.path.join(current_path, "stinity_mr-to-pct")
...
if __name__ == "__main__":
    ...
    if has_real_ct:
        ...
        if np.any(mask):
            # 1. MAE Global (El teu càlcul original)
            mae = np.mean(np.abs(pct_arr[mask] - ct_arr[mask]))
            
            # 2. Màscares específiques per a Teixit Tou i Os
            ...
            # 3. Càlcul del SSIM Global
            # Es calcula el rang dinàmic real basat en les dades del teixit del pacient
            data_range = float(np.max(ct_arr[mask]) - np.min(ct_arr[mask]))
            value_ssim = ssim_fn(ct_arr, pct_arr, data_range=data_range)

            # --- PRINT DE RESULTATS AL TERMINAL ---
            print("\n" + "-"*45)
    ...
    # 6. Visualization
    if has_real_ct:
        print("3. Opening 3D interactive viewer (4 panels)...")
        error_map_3d = np.abs(pct_arr - ct_arr)

        fig, axes = plt.subplots(1, 4, figsize=(20, 6))
        plt.subplots_adjust(bottom=0.2)
        # Afegim també l'SSIM al títol de la gràfica per tenir-ho visualment
