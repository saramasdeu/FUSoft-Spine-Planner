...
class MainWindow(QMainWindow):
    ...
    # ─────────────────────────────────────────────────────────────────────
    # CONIC BEAM SAMPLING
    # Helper: compute the perimeter ENTRY POINTS for a given central trajectory.
    # Reused for both metric computation and visualization (so the cone we
    # draw is exactly the cone we score).
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _run(self):
        ...
        for phi in phis:
            for theta in thetas:
                ...
                # Hard limit on the worst ray in the cone
                if bone_max > MAX_BONE_MM: continue

                # ── True incidence: ray vs real skin normal ──────────────
        ...
        # ─────────────────────────────────────────────────────────────────
        # Normalize metrics with ABSOLUTE references (not relative).
        # This way a "best of the worst" doesn't look artificially good.
        # ─────────────────────────────────────────────────────────────────
        DIST_REF    = 150.0
        BONE_REF    = MAX_BONE_MM
        ANGLE_REF   = 90.0
        CHANGES_REF = 100.0
        STD_REF     = 3.0          # mm of bone-std → considered very heterogeneous
        ...
        # ─── Quota: 5 + 5, with backfill if one pool is short ────────────
        QUOTA = 5
    ...
    def _update_3d(self, traj):
        ...
        # Apex at the target, base at the transductor entry point.
        # PyVista's Cone has apex along `direction` from the center.
